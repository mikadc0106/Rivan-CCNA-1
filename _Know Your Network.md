...
Task:
 1. CoreTAAS must have the following SVIs
   - VLAN 1
       - IP address: 10.22.1.2
       - Description: DEFAULT-VLAN
       - Status: UP

   - VLAN 10
     - IP address: 10.22.10.2
     - Description: WIFI-VLAN
     - Status: UP

   - VLAN 50
     - IP address: 10.22.50.2
     - Description: CCTV-VLAN
     - Status: UP

   - VLAN 100
     - IP address: 10.22.100.2
...
conf t
 int vlan 1
  ...
  exit
 int vlan 10
  ip add 10.22.10.2 255.255.255.0
  description WIFI-VLAN
  no shut
  exit
 int vlan 50
  ip add 10.22.50.2 255.255.255.0
  description CCTV-VLAN
  no shut
  exit
 int vlan 100
  ip add 10.22.100.2 255.255.255.0
  description VOICE-VLAN
  no shut
 end
~~~
...
conf t
 ...
 ip dhcp excluded-address 10.22.50.1 10.22.50.100
...
!@CoreBABA
conf t
 ip dhcp excluded-address 10.22.100.1 10.22.100.100
...
conf t
 ...
 ip route 10.22.0.0 255.255.0.0 10.22.22.4 254
 end
~~~
...
conf t
 ip routing
 ip route 0.0.0.0 0.0.0.0 10.22.100.4 254
 end
~~~
...
!@CoreBABA
conf t
 ip route 0.0.0.0 0.0.0.0 10.22.22.1 254
 end
~~~
...
~~~
!@cmd
route add 10.0.0.0 mask 255.0.0.0 10.22.1.4
